...
const packageBlank={name:'',description:'',includedTests:[],mrp:'',sellingPrice:'',imageUrl:'',featured:false,displayOrder:0,active:true}
...
function Form({ value, onChange, onSave, onCancel, kind, tests = [] }) {
  ...
  const catalogueFields = kind === 'Coupon' ? null : <div className="form-grid">
    ...
    {kind === 'Test' ? <>
      ...
      <label><input type="checkbox" checked={value.featured === true} onChange={e => set('featured', e.target.checked)} /> Featured on Homepage</label>
      <label>Display Order<input type="number" min="0" step="1" value={value.displayOrder ?? 0} onChange={e => set('displayOrder', e.target.value)} /></label>
    </>}
