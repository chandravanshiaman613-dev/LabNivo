...
const ADMIN_EDITABLE_STATUSES = ['NEW_BOOKING', 'CONFIRMED', 'COLLECTOR_ASSIGNED', 'COLLECTION_SCHEDULED', 'CANCELLED']
...
function publicBooking(booking) { return { bookingId: booking.bookingId, customer: booking.customer ? { name: booking.customer.name, mobile: booking.customer.mobile, alternateMobile: booking.customer.alternateMobile, age: booking.customer.age, gender: booking.customer.gender, address: booking.customer.address, city: booking.customer.city, district: booking.customer.district, state: booking.customer.state, pincode: booking.customer.pincode, landmark: booking.customer.landmark } : null, items: booking.items.map(item => ({ name: item.name, quantity: item.quantity, sellingPrice: item.sellingPrice })), subtotal: booking.subtotal ?? booking.totalAmount, couponCode: booking.couponCode || '', discountAmount: booking.discountAmount || 0, totalAmount: booking.totalAmount, finalAmount: booking.totalAmount, collectionCharge: booking.collectionCharge, preferredDate: booking.preferredDate, preferredTimeSlot: booking.preferredTimeSlot, paymentMethod: booking.paymentMethod || 'PAY_AT_COLLECTION', paymentStatus: booking.paymentStatus || 'PENDING', status: booking.status } }
...
export async function createBooking(request, response, next) {
  try {
    ...
    const finalAmount = Math.max(0, subtotal - coupon.discountAmount + collectionCharge)
    const booking = await Booking.create({ bookingId: await nextBookingId(), customer: customer._id, items: snapshots, totalMRP, totalDiscount: totalMRP - subtotal, subtotal, couponCode: coupon.couponCode, discountAmount: coupon.discountAmount, totalAmount: finalAmount, collectionCharge, address: [customerData.address, customerData.area, customerData.city, customerData.district, customerData.state, customerData.pincode].filter(Boolean).join(', '), preferredDate, preferredTimeSlot, notes: String(notes).trim().slice(0, 500), paymentMethod: 'PAY_AT_COLLECTION', paymentStatus: 'PENDING' })
...
export async function listAdminBookings(_request, response, next) { try { const data = await Booking.find().populate('customer').sort({ createdAt: -1 }); response.json({ success: true, data }) } catch (error) { next(error) } }
export async function getAdminBooking(request, response, next) { try { const data = await Booking.findOne({ bookingId: request.params.bookingId.toUpperCase() }).populate('customer'); if (!data) return response.status(404).json({ success: false, message: 'Booking not found' }); response.json({ success: true, data }) } catch (error) { next(error) } }
export async function updateBookingStatus(request, response, next) { try { const { status } = request.body; if (!ADMIN_EDITABLE_STATUSES.includes(status)) return response.status(400).json({ success: false, message: 'Invalid booking status.' }); const data = await Booking.findOneAndUpdate({ bookingId: request.params.bookingId.toUpperCase() }, { status }, { new: true }).populate('customer'); if (!data) return response.status(404).json({ success: false, message: 'Booking not found' }); response.json({ success: true, data }) } catch (error) { next(error) } }
